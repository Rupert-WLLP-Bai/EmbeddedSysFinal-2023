...
double LSTM::dotProduct(const std::vector<double>& a, const std::vector<double>& b) {
    double result = 0.0;
...
void LSTM::forward(const std::vector<double>& input) {
    ...
    for (int i = 0; i < hiddenSize; i++) {
        hiddenState[i] =
            dotProduct(input,
                       {&inputGateWeights[i * inputSize], &inputGateWeights[(i + 1) * inputSize]}) +
            inputGateBiases[i];
    }
    sigmoid(hiddenState);
    ...
    std::copy(input.begin(), input.end(), hiddenState.begin());
    for (int i = 0; i < hiddenSize; i++) {
        hiddenState[i] = dotProduct(input,
                                    {&forgetGateWeights[i * inputSize],
    ...
    for (int i = 0; i < hiddenSize; i++) {
        hiddenState[i] =
            dotProduct(input,
                       {&cellStateWeights[i * inputSize], &cellStateWeights[(i + 1) * inputSize]}) +
            cellStateBiases[i];
    }
    tanh(hiddenState);
...
void LSTM::backward(const std::vector<double>& dOutput) {
    ...
    for (int i = 0; i < inputSize; i++) {
        ...
        for (int j = 0; j < hiddenSize; j++) {
            dHiddenState[i] += dOutputGateBiases[j] * outputGateWeights[j * inputSize + i];
            dHiddenState[i] += dInputGateBiases[j] * inputGateWeights[j * inputSize + i];
            dHiddenState[i] += dForgetGateBiases[j] * forgetGateWeights[j * inputSize + i];
            dHiddenState[i] += dCellStateBiases[j] * cellStateWeights[j * inputSize + i];
        }
    }
}
...
std::vector<double> LSTM::getOutput() const {
    return output;
}
